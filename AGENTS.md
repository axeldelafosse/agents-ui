...
### Protocol docs
- `docs/codex.md`: Codex app-server JSON-RPC protocol, thread/turn lifecycle, events.
- `docs/claude.md`: Claude Code SDK URL protocol, session rotation, relay architecture.
...
- Codex: one shared hub per URL, many logical threads.
- Auto-discovery probes:
- codex `4500-4509`
- claude `8765-8774`

### Editing guardrails
- Keep changes small and verify with `bun run build`.
- If changing Codex lifecycle code, test early-close and reconnect cases.
- Keep the generated Next docs index block below (`NEXT-AGENTS-MD-*`) intact.
- After upgrade Next.js, refresh the generated docs block with: `bun run next-agents-md`
