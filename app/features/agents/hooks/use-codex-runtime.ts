...
export function useCodexRuntime({
  ...
  const requestCodexLoadedList = useCallback((hub: CodexHub) => {
  ...
  const routeCodexResponse = useCallback(
    ...
    (hub: CodexHub, msg: CodexRpcMessage) => {
      ...
      if (ctx.type === "initialize" && msg.result) {
        ...
        hub.ws.send(JSON.stringify({ jsonrpc: "2.0", method: "initialized" }))
  ...
  const getOrCreateCodexHub = useCallback(
    (
      ...
      ws.onclose = () => {
            ...
            prev
              ...
              .map((a) =>
                a.url === targetUrl && a.protocol === "codex"
                  ? { ...a, status: "disconnected" as Status }
                  : a
              )
          )
          return
        ...
        // Freeze existing tabs on disconnect. A reconnected hub will discover
